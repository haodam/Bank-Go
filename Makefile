...
migrateup1:
	migrate -path simplebank/db/migration -database "postgresql://root:secret@localhost:5432/simple_bank?sslmode=disable" -verbose up 1
...
server:
	go run main.go
mock:
	mockgen -package mockdb -destination simplebank/db/mock/store.go github.com/haodam/Bank-Go/simplebank/db/sqlc Store
